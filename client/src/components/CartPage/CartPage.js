...
const CartPage = () => {
    ...
    const calculateItemPrice = (item) => {
        if (item.itemType === 'addon') {
            ...
            // Для цветов: используем itemTotal *item-price количество
            return (+item.itemTotal || 0);
        }
    };

    // Функция для отображения информации о выбранном цвете
    // В функции renderColorInfo обновите:
        ...
        if (item.itemType === 'flower' && item.selectedColor) {
            return (
                <div className="item-color-info">
                    ...
                    <span style={{color: item.selectedColor.value }} className="color-name">
                    {item.selectedColor.name || 'Не указан'}
                </span>
                </div>
            );
        } else if (item.itemType === 'flower' && item.flowerType === 'single') {
            return (
                <div className="item-color-info">
                    <span className="color-label">Цвет:</span>
                    <span className="color-name not-selected">Не выбран</span>
                </div>
            );
        }
        return null;
                                    ...
                                    <div className="item-details">
                                        ...
                                        {item.itemType === 'flower' && (
                                            <div className="item-specs">
                                                ...
                                                {item.flowerNames && item.flowerNames== null && item.flowerNames.length > 0 && (
