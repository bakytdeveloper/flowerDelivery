...
// ОБНОВЛЕННЫЙ Контроллер для создания продукта
export const createProduct = async (req, res) => {
    try {
        const {
            ...
            recipient,
            flowerNames,
            stemLength, // оставляем для обратной совместимости
            stemLengths, // новые варианты длин стеблей
            flowerColors,
            availableColors, // новые доступные цвета
            characteristics,
            images,
            quantity = 10
        } = req.body;
        ...
        if (!name || !type) {
            return res.status(400).json({
                message: 'Необходимо заполнить все обязательные поля: название и тип'
            });
        }

        // Проверяем наличие вариантов длины стебля
        if (!stemLengths || !Array.isArray(stemLengths) || stemLengths.length === 0) {
            return res.status(400).json({
                message: 'Необходимо добавить хотя бы один вариант длины стебля'
            });
        }
        ...
        // Обрабатываем flowerColors (для обратной совместимости)
        ...
        // Обрабатываем availableColors
        let processedColors = [];
        if (availableColors && Array.isArray(availableColors)) {
            processedColors = availableColors.map(color => ({
                name: color.name || '',
        ...
        // Обрабатываем stemLengths и вычисляем основную цену
        let processedStemLengths = [];
        let basePrice = 0;

        if (stemLengths && Array.isArray(stemLengths)) {
            processedStemLengths = stemLengths.map(item => ({
                length: Number(item.length) || 0,
                price: Number(item.price) || 0,
                originalPrice: item.originalPrice ? Number(item.originalPrice) : undefined
            }));

            // Основная цена = минимальная цена из stemLengths
            basePrice = Math.min(...processedStemLengths.map(item => item.price));
        }

        // Если stemLengths не предоставлены, используем старый stemLength
        let finalStemLength = 0;
        if (processedStemLengths.length > 0) {
            finalStemLength = processedStemLengths[0].length; // используем первую длину как основную
        } else if (stemLength) {
            finalStemLength = parseInt(stemLength);
        }

        const newProduct = new Product({
            name,
            description,
            price: basePrice, // используем вычисленную базовую цену
            originalPrice,
            type,
            occasion,
            recipient,
            flowerNames: processedFlowerNames,
            stemLength: finalStemLength,
            stemLengths: processedStemLengths,
...
// Функция для получения доступных цветов и длин
export const getProductVariants = async (req, res) => {
    try {
        const { productId } = req.params;

        const product = await Product.findById(productId)
            .select('type availableColors stemLengths flowerColors');

        if (!product) {
            return res.status(404).json({ message: 'Товар не найден' });
        }

        // Для обратной совместимости объединяем availableColors и flowerColors
        const allColors = [
            ...(product.availableColors || []),
            ...(product.flowerColors || [])
        ];

        res.json({
            type: product.type,
            availableColors: allColors,
            stemLengths: product.stemLengths || []
        });
    } catch (error) {
...
// ОБНОВЛЕННЫЙ Контроллер для обновления продукта
export const updateProduct = async (req, res) => {
    try {
        ...
        if (updateData.availableColors && Array.isArray(updateData.availableColors)) {
            updateData.availableColors = updateData.availableColors.map(color => ({
                name: color.name || '',
                value: color.value || '#000000',
                colorImages: color.colorImages || []
            }));
        ...
        // Обрабатываем stemLengths и обновляем основную цену
        if (updateData.stemLengths && Array.isArray(updateData.stemLengths)) {
            updateData.stemLengths = updateData.stemLengths.map(item => ({
                length: Number(item.length) || 0,
                price: Number(item.price) || 0,
                originalPrice: item.originalPrice ? Number(item.originalPrice) : undefined
            }));

            // Обновляем основную цену = минимальная цена из stemLengths
            updateData.price = Math.min(...updateData.stemLengths.map(item => item.price));

            // Обновляем основную длину стебля (первая в списке)
            if (updateData.stemLengths.length > 0) {
                updateData.stemLength = updateData.stemLengths[0].length;
            }
        }
        ...
        if (updateData.stemLength && !updateData.stemLengths) {
    ...
    try {
        ...
        if (product.availableColors && Array.isArray(product.availableColors)) {
            for (const color of product.availableColors) {
                if (color.colorImages && Array.isArray(color.colorImages)) {
                    for (const colorImageUrl of color.colorImages) {
                        const colorImagePath = path.join(__dirname, '..', 'uploads', path.basename(colorImageUrl));
