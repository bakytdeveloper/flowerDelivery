...
//             quantity,
//             flowerType,
//             sselectedColor, // Может быть undefined
//             selectedStemLength, // Может быть undefined
//             wrapper
//         } = req.body;
//
//         const { user } = req;
...
// В функции addFlowerToCart добавьте автоматическую установку цвета и длины
export const addFlowerToCart = async (req, res) => {
    try {
        const {
            productId,
            quantity,
            flowerType,
            selectedColor, // Может быть undefined
            selectedStemLength, // Может быть undefined
            wrapper
        } = req.body;
        ...
        // ДЛЯ ШТУЧНЫХ ЦВЕТОВ: устанавливаем цвет по умолчанию если не выбран
        let finalSelectedColor = selectedColor;
        if (flowerType === 'single' && !selectedColor && product.availableColors && product.availableColors.length > 0) {
            finalSelectedColor = {
                name: product.availableColors[0].name,
                value: product.availableColors[0].value,
                image: product.availableColors[0].colorImages && product.availableColors[0].colorImages.length > 0
                    ? product.availableColors[0].colorImages[0]
                    : null
            };
        }

        // ДЛЯ ВСЕХ ТИПОВ: устанавливаем длину стебля по умолчанию если не выбрана
        let finalSelectedStemLength = selectedStemLength;
        let stemPrice = product.price;
        let selectedLength = product.stemLength;

        if (!selectedStemLength && product.stemLengths && product.stemLengths.length > 0) {
            const defaultStem = product.stemLengths[0];
            finalSelectedStemLength = {
                length: defaultStem.length,
                price: defaultStem.price
            };
            stemPrice = defaultStem.price;
            selectedLength = defaultStem.length;
        } else if (selectedStemLength && product.stemLengths && product.stemLengths.length > 0) {
            const stemOption = product.stemLengths.find(
                item => item.length === selectedStemLength.length
            );
            if (!stemOption) {
        ...
        // Проверяем выбранный цвет для штучных цветов
        if (flowerType === 'single' && finalSelectedColor) {
            const colorExists = product.availableColors && product.availableColors.some(
                color => color.value === finalSelectedColor.value
            );
            if (!colorExists) {
                return res.status(400).json({ message: 'Выбранный цвет недоступен' });
            }
        ...
        const flowerItem = {
            ...
            quantity,
            flowerType,
            selectedColor: flowerType === 'single' ? finalSelectedColor : undefined,
            selectedStemLength: finalSelectedStemLength ? {
                length: finalSelectedStemLength.length,
                price: stemPrice
            } : undefined,
            price: unitPrice,
            name: product.name,
            image: getProductImage(product, finalSelectedColor),
            flowerNames: product.flowerNames,
            stemLength: selectedLength,
            wrapper: wrapperData,
            itemTotal
