...
async function sendOrderEmail(order, userType) {
    try {
        ...
        // Безопасное форматирование списка цветов с учетом цветов и длин стеблей
            ...
            html: `
                ...
                <head>
                    ...
                    <style>
                        ...
                        .variant-info { color: #666; font-size: 0.9em; margin-left: 10px; }
                    </style>
                </head>
                <body>
                    <div class="container">
    ...
    try {
        ...
        if (user.userId && user.userId !== 'admin') {
            orders = await Order.find({ user: user.userId })
                .sort({ date: -1 })
                .populate('flowerItems.product', 'name images price flowerNames stemLength occasion recipient type description availableColors stemLengths')
        ...
        const formattedOrders = orders.map(order => ({
            ...
            flowerItems: order.flowerItems.map(item => ({
                ...item.toObject(),
                // Включаем выбранные цвета и длины стеблей
                selectedColor: item.selectedColor || null,
                selectedStemLength: item.selectedStemLength || null,
                product: item.product ? {
                    ...
                    stemLengths: item.product.stemLengths || []
                } : null
            })),
    ...
    try {
        return {
            ...
            flowerItems: order.flowerItems.map(item => ({
                ...
                // Включаем информацию о выбранном цвете и длине стебля
                selectedColor: item.selectedColor || null,
                selectedStemLength: item.selectedStemLength || null,
                product: item.product ? {
                    _id: item.product._id,
                    ...
                    type: item.product.type,
                    description: item.product.description,
                    // Включаем доступные цвета и длины для продукта
                    availableColors: item.product.availableColors || [],
                    stemLengths: item.product.stemLengths || []
                } : null
            })),
    ...
    try {
        ...
        for (const item of order.flowerItems) {
            if (item.product) {
                ...
                console.log(`✅ Возвращены цветы: ${item.name}, количество: ${item.quantity}, цвет: ${item.selectedColor?.name}, длина: ${item.selectedStemLength?.length}см`);
            }
        }
...
async function deductOrderItemsFromStock(order) {
    try {
        ...
        for (const item of order.flowerItems) {
            ...
            console.log(`✅ Списан товар: ${item.name}, количество: ${item.quantity}, цвет: ${item.selectedColor?.name}, длина: ${item.selectedStemLength?.length}см`);
        }
